...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	switch ($_POST['action']) {
		case 'rem':
			$result = mysqli_query($conn, Jogo::deleteConsoleByGameIDAndConsoleID($_POST['gameID'], $_POST['genID']));
			if ($result) {
			...
			break;
		case 'inc':
			$result = mysqli_query($conn, Jogo::insertGameConsole($_POST['gameID'], $_POST['genID']));
			if ($result) {
				$rows = mysqli_affected_rows($conn);
				if ($rows > 0) {
	...
	switch ($_GET['action']) {
		case 'rem':
			$result = mysqli_query($conn, Jogo::deleteConsoleByGameIDAndConsoleID($_GET['gameID'], $_GET['genID']));
			if ($result) {
				$rows = mysqli_affected_rows($conn);
				if ($rows > 0) {
					$return['msg'] = "Remoção eftuada com sucesso. Linhas afetadas: " . $rows;
			...
			break;
		case 'inc':
			$result = mysqli_query($conn, Jogo::insertGameConsole($_GET['gameID'], $_GET['genID']));
			if ($result) {
				$rows = mysqli_affected_rows($conn);
				if ($rows > 0) {
					$return['msg'] = "Inclusão eftuada com sucesso. Linhas afetadas: " . $rows;
